...
def learn(win, storedstates, storedmoves):
    ...
    if win == 'True':
        for item in storedmoves:
            weightchange(cells, weights, storedstates[it], storedmoves[it], 10) #WIN BONUS
            it += 1
    else:
        for item in storedmoves:
            weightchange(cells, weights, storedstates[it], storedmoves[it], -1) #LOSS PENALTY
            it += 1
...
while not closed:
    ...
    if click[0] == 1 and turn == 'player' and train == 'no':
        ...
        if cells[cell-1] == 3:
            placethree = cellconvert(cell)
            draw(cross, placethree[0], placethree[1])
            win = checkwin(cells)
            if win != 'False':
                winner = win
                if winner == 2:
